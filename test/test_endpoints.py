...
class TestRecordLearningEndpoint:
    ...
    def test_record_learning_saves_to_dynamodb(self, dynamodb_table):
        ...
        item = dynamodb_table.get_item(Key={"PK": f"USER#{TEST_USER}", "SK": "WORD#7"}).get("Item")
...
class TestGetNextWordEndpoint:
    ...
    def test_next_word_with_words_in_db(self, dynamodb_table):
        ...
        for w in [
            {"id": 1, "name": "犬", "english": "dog"},
            {"id": 2, "name": "猫", "english": "cat"},
            {"id": 3, "name": "鳥", "english": "bird"},
            {"id": 4, "name": "魚", "english": "fish"},
        ]:
    ...
    def test_next_word_review_all_with_history(self, dynamodb_table):
        ...
        insert_words(dynamodb_table, [{"id": i, "name": f"word{i}", "level": 1} for i in range(1, 10)])
        for i in range(1, 10):
            dynamodb_table.put_item(
                Item={
                    "PK": "WORD",
