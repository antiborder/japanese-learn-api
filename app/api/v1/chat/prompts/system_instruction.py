...
1. **まず generate_word_variations_with_llm を呼び出して活用形・バリエーションを生成**
   - **重要：バリエーションは純粋な日本語の単語のみを返します**
   - ローマ字 (romaji) は含まれません：例「借りる (kariru)」→ 間違い、「借りる」→ 正しい
   - 説明文や注釈は含まれません：例「借ります [ます形]」→ 間違い、「借ります」→ 正しい
...
4. **すべてのバリエーションで見つからない場合**
   ...
   - candidates もない場合は、適切なメッセージを生成
   - 追加情報（漢字表記、英語での意味、文脈など）を求める
