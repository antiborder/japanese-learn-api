...
class FAISSVectorStore:
            ...
            while True:
                ...
                for item in items:
                    ...
                    if embedding and isinstance(embedding, list) and len(embedding) == 1536:
                        ...
                        mapping_data = {"pk": item["PK"], "sk": item["SK"], "entity_type": pk.lower()}
                        ...
                        if pk == "WORD":
                            ...
                            mapping_data.update(
                                {"japanese": item.get("japanese", ""), "english": item.get("english", "")}
                            )
    ...
    def save_to_s3(self):
        ...
        # Save metadata
        metadata = {"id_mapping": self.id_mapping, "total_vectors": self.index.ntotal}
        ...
        # Upload to S3
        try:
            self.s3_client.upload_file("/tmp/faiss_index/index.faiss", self.s3_bucket, self.index_key)

            self.s3_client.upload_file("/tmp/faiss_index/metadata.pkl", self.s3_bucket, self.metadata_key)
    ...
    def load_from_s3(self) -> bool:
        ...
        try:
            ...
            s3_client.download_file(self.s3_bucket, self.index_key, "/tmp/faiss_index/index.faiss")
            logger.info(f"Index file downloaded in {time.time() - download_start:.2f} seconds")

            logger.info("Downloading metadata file from S3...")
            s3_client.download_file(self.s3_bucket, self.metadata_key, "/tmp/faiss_index/metadata.pkl")
    ...
    def similarity_search(self, query: str, k: int = 5, entity_type: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            for i, (distance, idx) in enumerate(zip(distances[0], indices[0])):
                ...
                result = {"id": word_id, "score": float(distance), "entity_type": entity_type_from_mapping}
                ...
                if entity_type_from_mapping == "word":
                    ...
                    if not name or not hiragana or not english:
                        ...
                        try:
                            ...
                            if item:
                                ...
                                level = item.get("level", 0)
                                logger.info(
                                    f"Fetched from DynamoDB: name={name}, hiragana={hiragana}, english={english}"
                                )
                        except Exception as e:
                    ...
                    result.update(
                        {"name": str(name), "hiragana": str(hiragana), "english": str(english), "level": level}
                    )
                    ...
                    result.update(
                        {"japanese": str(mapping.get("japanese", "")), "english": str(mapping.get("english", ""))}
                    )
            ...
            logger.info(
                f"Similarity search returned {len(results)} results for query '{query}' (entity_type: {entity_type})"
            )
            if results:
                logger.info(
                    f"Best match: {results[0].get('name', results[0].get('kanji', 'unknown'))} (score: {results[0].get('score', 0):.4f})"
                )
