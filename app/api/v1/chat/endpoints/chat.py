...
async def chat_message(request: ChatMessageRequest, user_id: str = Depends(get_current_user_id)):
    """
    ...
    try:
        ...
        if session_id:
            try:
                ...
                if history:
                    ...
                    logger.info(
                        f"No conversation history found for session {session_id} (this is expected for first message)"
                    )
        ...
        if result.get("tool_results"):
            ...
            for i, tr in enumerate(result["tool_results"]):
                ...
                if "result" in tr:
                    logger.info(
                        f"  Result keys: {tr['result'].keys() if isinstance(tr['result'], dict) else 'not a dict'}"
                    )
                    if isinstance(tr["result"], dict) and "candidates" in tr["result"]:
                        logger.info(
                            f"  Candidates keys: {tr['result']['candidates'].keys() if isinstance(tr['result']['candidates'], dict) else 'not a dict'}"
                        )
        else:
        ...
        if result.get("tool_results"):
            ...
            for i, tool_result in enumerate(result["tool_results"]):
                logger.info(
                    f"Tool result {i}: name={tool_result.get('name')}, keys={list(tool_result.keys()) if isinstance(tool_result, dict) else 'not a dict'}"
                )
                ...
                logger.info(
                    f"Processing tool result {i} data: {list(result_data.keys()) if isinstance(result_data, dict) else 'not a dict'}"
                )
                ...
                if "word" in result_data and result_data.get("word"):
                    ...
                    if "id" in word:
                        ...
                        logger.warning(
                            f"Word in result {i} has no 'id' field: {word.keys() if isinstance(word, dict) else 'not a dict'}"
                        )
                ...
                if "kanji" in result_data and result_data.get("kanji"):
                    ...
                    if "id" in kanji:
                        ...
                        logger.warning(
                            f"Kanji in result {i} has no 'id' field: {kanji.keys() if isinstance(kanji, dict) else 'not a dict'}"
                        )
                ...
                if "candidates" in result_data and result_data.get("candidates"):
                    ...
                    logger.info(
                        f"Found candidates: {list(candidates.keys()) if isinstance(candidates, dict) else 'not a dict'}"
                    )
                    ...
                    if "combined" in candidates and candidates.get("combined"):
                        ...
                        for candidate in candidates["combined"]:
                            ...
                            logger.info(
                                f"  Candidate: type={repr(candidate_type)}, id={candidate_id}, full candidate: {candidate}"
                            )
                            if candidate_id is not None:
                                if candidate_type == "word" and candidate_id not in word_ids:
                                    ...
                                    logger.warning(
                                        f"  Skipped candidate: type={repr(candidate_type)}, id={candidate_id}"
                                    )
        ...
        logger.info(
            f"Final response - word_ids: {response_data.get('word_ids')}, kanji_ids: {response_data.get('kanji_ids')}"
        )
