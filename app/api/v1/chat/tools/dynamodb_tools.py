    ...
    try:
        ...
        try:
            ...
            for v in ordered[:10]:
                ...
                if exact_v:
                    logger.info(
                        f"Word match found via variation. target='{target}' matched='{v}' result='{exact_v.get('name')}'"
                    )
                    return {
                        "found": True,
                        "word": {
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    try:
                        ...
                        if not combined_candidates:
                            return {"found": False, "word": None, "candidates": None, "message": "No results found."}
...
def get_word_by_id(word_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if item:
            ...
            return {"found": False, "word": None, "message": f"Word with ID {word_id} not found"}

    except Exception as e:
        logger.error(f"Error getting word {word_id}: {str(e)}")
        return {"found": False, "word": None, "message": f"Error getting word: {str(e)}"}
...
def search_kanji_by_character(kanji_character: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"found": False, "kanji": None, "message": f"Kanji '{kanji_char}' not found"}

    except Exception as e:
        logger.error(f"Error searching kanji '{kanji_character}': {str(e)}")
        return {"found": False, "kanji": None, "message": f"Error searching for kanji: {str(e)}"}
...
def generate_word_variations_with_llm(word_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(client.generate_word_variations, word_name, lang="ja", max_variations=10)
            try:
                ...
                return {"variations": [], "reasoning": "Generation timed out", "confidence": 0.0}
...
TOOL_FUNCTIONS = {
    ...
    "get_word_by_id": {
        ...
        "parameters": {
            "type": "object",
            "properties": {"word_id": {"type": "integer", "description": "The word ID"}},
            "required": ["word_id"],
        },
    },
