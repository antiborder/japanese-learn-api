...
class NextDynamoDB(DynamoDBBase):
    ...
    async def batch_get_word_details(self, word_ids: List[int]) -> Dict[int, Optional[dict]]:
        ...
        try:
            ...
            # 通常は4つ程度なので問題なし
            keys = [{"PK": {"S": "WORD"}, "SK": {"S": str(word_id)}} for word_id in word_ids]
            ...
            for item in items:
                ...
                result[word_id] = {
                    ...
                    "accent_up": int(deserialized_item["accent_up"])
                    if deserialized_item.get("accent_up") is not None
                    else None,
                    "accent_down": int(deserialized_item["accent_down"])
                    if deserialized_item.get("accent_down") is not None
                    else None,
                }
        ...
        try:
            ...
            while True:
                query_params = {
                    ...
                    "ExpressionAttributeValues": {":pk": "WORD", ":level": int(level)},
    ...
    async def _get_user_words(self, user_id: str) -> List[Dict]:
        ...
        response = self.table.query(
            ...
            ExpressionAttributeValues={":pk": f"USER#{user_id}", ":sk_prefix": "WORD#"},
        )
    ...
    async def _get_user_words_by_level(self, user_id: str, level: int) -> List[Dict]:
        ...
        try:
            response = self.table.query(
                ...
                KeyConditionExpression="PK = :pk AND #level = :level",
                ExpressionAttributeNames={"#level": "level"},
                ExpressionAttributeValues={":pk": f"USER#{user_id}", ":level": int(level)},
            )
            user_level_words = response.get("Items", [])
            # SKがWORD#で始まるものだけをフィルタリング（念のため）
            filtered_words = [item for item in user_level_words if item.get("SK", "").startswith("WORD#")]
            if not filtered_words:
                logger.info(f"No learning history found for user {user_id}, level {level}")
                return []

            logger.info(
                f"Successfully retrieved {len(filtered_words)} learning history items for user {user_id}, level {level}"
            )
