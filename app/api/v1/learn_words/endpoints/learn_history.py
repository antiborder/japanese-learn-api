...
async def get_next_word(request: NextWordRequest, current_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        # 1. 出題単語IDとモード取得
        next_result = await next_service.get_next_word(user_id=current_user_id, level=request.level)
        if not next_result:
            raise HTTPException(status_code=404, detail="No words found for the specified level")

        # 単語がない場合のレスポンスをチェック
        if next_result.get("no_word_available"):
            return NoWordAvailableResponse(next_available_datetime=next_result.get("next_available_datetime"))
        ...
        if len(other_words) < 3 and missing_word_ids:
            logger.info(
                f"Only {len(other_words)} valid words found, fetching additional words excluding {missing_word_ids}"
            )
            additional_exclude_ids = [answer_word_id] + [w["id"] for w in other_words] + missing_word_ids
            additional_word_ids = await next_service.get_other_words(
                request.level, answer_word_id, additional_exclude_ids
            )
        ...
        if len(other_words) < 3:
            raise HTTPException(
                status_code=404, detail=f"Not enough valid words found. Only {len(other_words)} valid words available."
            )
