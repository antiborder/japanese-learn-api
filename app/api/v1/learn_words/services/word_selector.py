...
class WordSelector:
    ...
    def select_new_word(level_words: List[Dict], user_words: List[Dict]) -> Optional[Dict]:
        ...
        new_words = [word for word in level_words if int(word["SK"]) not in user_learned_ids]
        ...
        return {"answer_word_id": int(selected_item["SK"]), "mode": random.choice(["MJ", "JM"])}
    ...
    def select_review_word(
        ...
        if existing_word_ids is not None:
            ...
            reviewable_words = [w for w in reviewable_words if int(w["word_id"]) in existing_word_ids]
        ...
        return {"answer_word_id": answer_word["word_id"], "mode": answer_word["next_mode"]}
    ...
    def select_next_word(
        level_words: List[Dict],
        user_words: List[Dict],
        user_id: str,
        level: Union[int, str],
        user_level_words: Optional[List[Dict]] = None,
    ) -> Optional[Dict]:
        """次に学習すべき単語を選択します"""
        ...
        review_word_result = WordSelector.select_review_word(user_level_words, existing_word_ids=level_word_ids)
        if review_word_result:
            logger.info(
                f"Successfully retrieved review word for user {user_id}, level {level_int}: {review_word_result}"
            )
        ...
        if new_word_result:
            logger.info(
                f"No review available, retrieved new word for user {user_id}, level {level_int}: {new_word_result}"
            )
            return new_word_result
        ...
        if next_available_dt:
            result = {"no_word_available": True, "next_available_datetime": next_available_dt}
            logger.info(
                f"No reviewable words available for user {user_id}, level {level_int}. Next available at: {next_available_dt}"
            )
            return result
