...
class SentenceSelector:
    ...
    def select_new_sentence(level_sentences: List[Dict], user_sentences: List[Dict]) -> Optional[Dict]:
        ...
        new_sentences = [sentence for sentence in level_sentences if int(sentence["SK"]) not in user_learned_ids]
        ...
        return {"answer_sentence_id": int(selected_item["SK"])}
    ...
    def select_review_sentence(user_level_sentences: List[Dict]) -> Optional[Dict]:
        ...
        return {"answer_sentence_id": answer_sentence["sentence_id"]}

    @staticmethod
    def select_next_sentence(
        level_sentences: List[Dict], user_sentences: List[Dict], user_id: str, level: int
    ) -> Optional[Dict]:
        """次に学習すべき文を選択します"""
        ...
        if random.random() > ratio:
            ...
            if new_sentence_result:
                logger.info(
                    f"Successfully retrieved new sentence for user {user_id}, level {level}: {new_sentence_result}"
                )
        ...
        if review_sentence_result:
            logger.info(
                f"Successfully retrieved review sentence for user {user_id}, level {level}: {review_sentence_result}"
            )
        ...
        if new_sentence_result:
            logger.info(
                f"No review available, retrieved new sentence for user {user_id}, level {level}: {new_sentence_result}"
            )
            return new_sentence_result
        ...
        if next_available_dt:
            result = {"no_sentence_available": True, "next_available_datetime": next_available_dt}
            logger.info(
                f"No reviewable sentences available for user {user_id}, level {level}. Next available at: {next_available_dt}"
            )
            return result
        ...
        if level_sentences:
            ...
            random_result = {"answer_sentence_id": int(selected_item["SK"])}
