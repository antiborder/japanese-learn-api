...
class NextService:
    ...
    async def get_next_sentence(self, user_id: Optional[str], level: int) -> Optional[Dict]:
        ...
        try:
            ...
            # ③文選定
            result = self.sentence_selector.select_next_sentence(level_sentences, user_sentences, user_id, level)
            ...
            if result.get("no_sentence_available"):
                return {"no_sentence_available": True, "next_available_datetime": result["next_available_datetime"]}
