...
class LearningService:
    ...
    async def record_learning(self, user_id: str, sentence_id: int, level: int, confidence: int, time: Decimal) -> Dict:
        """学習履歴を記録します（ビジネスロジック）"""
        try:
            # ユーザーIDがない場合は記録をスキップ
            if not user_id:
            ...
            if current_data:
                _proficiency = Decimal(str(current_data.get("proficiency", "0")))
            else:
                _proficiency = Decimal("0")
