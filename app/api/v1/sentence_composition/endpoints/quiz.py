...
@router.get("/random")
async def get_random_sentence(level: int = Query(..., description="文のレベル（1-15）", ge=1, le=15)):
    """
...
async def record_sentence_attempt(
    user_id: str = Path(..., description="ユーザーID"), request: SentenceAttemptRequest = ...
):
    """
    文の学習履歴を記録します
...
async def get_next_sentence(
    ...
    try:
        ...
        if result.get("no_sentence_available"):
            return NoSentenceAvailableResponse(next_available_datetime=result.get("next_available_datetime"))
