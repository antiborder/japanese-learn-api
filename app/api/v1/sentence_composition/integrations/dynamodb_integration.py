...
class DynamoDBSentenceCompositionClient:
        ...
        try:
            ...
            while True:
                query_params = {
                    ...
                    "ExpressionAttributeValues": {":pk": "SENTENCE", ":level": int(level)},
        ...
        if "words" in item and item["words"]:
            for word_item in item["words"]:
                if isinstance(word_item, dict):
                    ...
                    words.append({"word_id": word_id, "word_name": word_name})
    ...
    def get_current_learning_data(self, user_id: str, sentence_id: int) -> Optional[Dict]:
        """現在の学習データを取得します"""
        try:
            response = self.table.get_item(Key={"PK": f"USER#{user_id}", "SK": f"SENTENCE#{sentence_id}"})
    ...
    async def save_learning_data(
        self, user_id: str, sentence_id: int, level: int, proficiency: Decimal, next_datetime: datetime
    ) -> Dict:
    ...
    async def get_user_sentences(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = self.table.query(
                ...
                ExpressionAttributeValues={":pk": f"USER#{user_id}", ":sk_prefix": "SENTENCE#"},
            )
