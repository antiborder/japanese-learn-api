...
def get_kanji_ai_description(kanji_id: int, kanji_character: str, lang_code: str = "en") -> str:
    ...
    try:
        logger.info(
            f"Getting AI description for kanji_id: {kanji_id}, kanji_character: {kanji_character}, lang: {lang_code}"
        )
        ...
        logger.info("AI description not found in S3, generating with Gemini API")
        ...
        try:
            ...
            if not description_text:
                raise HTTPException(status_code=500, detail="Failed to generate AI description: Empty response")
