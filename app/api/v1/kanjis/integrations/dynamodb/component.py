...
class DynamoDBComponent:
    ...
    def get_components(self, skip: int = 0, limit: int = 100) -> List[dict]:
        try:
            ...
            while True:
                query_params = {"KeyConditionExpression": "PK = :pk", "ExpressionAttributeValues": {":pk": "COMPONENT"}}
        ...
        try:
            ...
            while True:
                query_params = {
                    ...
                    "ExpressionAttributeValues": {":pk": "COMPONENT"},
                    "Select": "COUNT",
                }
    ...
    def get_component(self, component_id: str) -> Optional[dict]:
        try:
            response = self.table.get_item(Key={"PK": "COMPONENT", "SK": component_id})
    ...
    def get_kanjis_by_component_id(self, component_id: str):
        try:
            response = self.table.query(
                KeyConditionExpression="PK = :pk", ExpressionAttributeValues={":pk": f"COMPONENT#{component_id}"}
            )
            items = response.get("Items", [])
            result = []
            for item in items:
                ...
                result.append({"kanji_id": kanji_id, "kanji_char": kanji_char})
            return result
