...
async def fetch_sentence_audio(sentence_id: int):
    ...
    try:
        ...
        # 音声URLを取得
        audio_url = get_sentence_audio_url(sentence_id, sentence.get("japanese"), sentence.get("hurigana", ""))
...
async def fetch_ai_grammar_description(
    sentence_id: int, lang: Optional[str] = Query(default="en", description="言語コード (en, vi, zh-Hans, hi, etc.)")
):
    """
