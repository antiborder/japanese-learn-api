...
def get_sentence_grammar_description(sentence_id: int, sentence_text: str, level: int, lang_code: str = "en") -> str:
    ...
    try:
        logger.info(
            f"Getting AI grammar description for sentence_id: {sentence_id}, sentence: {sentence_text}, level: {level}, lang: {lang_code}"
        )
        ...
        logger.info("AI grammar description not found in S3, generating with Gemini API")

        if not sentence_text:
            raise HTTPException(
                status_code=404, detail="Sentence text is required for AI grammar description generation"
            )
        ...
        try:
            ...
            if not description_text:
                raise HTTPException(status_code=500, detail="Failed to generate AI grammar description: Empty response")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to generate AI grammar description: {str(e)}")

        # ステップ3: 生成したAI解説をS3に保存
        try:
            save_ai_description_to_s3(sentence_id, lang_code, description_text)
            logger.info(
                f"Successfully saved AI grammar description to S3 for sentence_id: {sentence_id}, lang: {lang_code}"
            )
        except Exception as e:
