...
class SearchService:
    ...
    async def search_words(self, request: SearchRequest) -> SearchResponse:
        ...
        try:
            ...
            words_data = self.dynamodb_client.search_words(
                query=request.query, language=request.language.value, limit=request.limit, offset=request.offset
            )

            # 総数を取得
            total_count = self.dynamodb_client.get_total_count(query=request.query, language=request.language.value)
    ...
    async def search_kanjis(self, query: str, limit: int = 20, offset: int = 0) -> List[KanjiSearchResult]:
        ...
        try:
            ...
            # 検索実行
            kanjis_data = self.dynamodb_client.search_kanjis(query=query, limit=limit, offset=offset)
    ...
    async def search_components(self, query: str, limit: int = 20, offset: int = 0) -> List[ComponentSearchResult]:
        ...
        try:
            ...
            # 検索実行
            components_data = self.dynamodb_client.search_components(query=query, limit=limit, offset=offset)

            # 結果を変換
            components = []
            for component_data in components_data:
                component = ComponentSearchResult(id=component_data["id"], character=component_data["character"])
    ...
    async def search_all(self, query: str, language: Language, limit: int = 20, offset: int = 0) -> Dict:
        ...
        try:
            ...
            # 単語検索
            request = SearchRequest(query=query, language=language, limit=limit, offset=offset)
