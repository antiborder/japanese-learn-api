...
async def search_words(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid language code. Must be 'en', 'vi', 'zh-Hans', 'ko', 'id', or 'hi'"
            )

        # リクエスト作成
        request = SearchRequest(query=q, language=language, limit=limit, offset=offset)
...
async def search_all(
    ...
    try:
        ...
        # 統合検索実行
        result = await search_service.search_all(query=q, language=language, limit=limit, offset=offset)
