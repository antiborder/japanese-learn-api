...
def save_word_audio_to_s3(word_id: int, audio_content: bytes):
    try:
        ...
        s3_client.put_object(Bucket=bucket_name, Key=object_key, Body=audio_content, ContentType="audio/mpeg")
...
def save_word_image_to_s3(word_id: int, image_index: int, image_content: bytes, extension: str):
    ...
    try:
        ...
        s3_client.put_object(Bucket=bucket_name, Key=object_key, Body=image_content, ContentType=content_type)
...
def check_ai_description_exists(word_id: int, lang_code: str) -> bool:
    """
    S3にAI解説が存在するかチェック
    ...
    """
    try:
        object_key = f"ai_descriptions/words/{word_id}_{lang_code}.txt"
...
def get_ai_description_from_s3(word_id: int, lang_code: str) -> str:
    """
    S3からAI解説テキストを取得
...
def save_ai_description_to_s3(word_id: int, lang_code: str, description_text: str):
    """
    AI解説テキストをS3に保存
