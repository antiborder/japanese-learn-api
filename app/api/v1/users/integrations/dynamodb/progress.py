    ...
    def _get_all_words_with_pagination(self) -> List[Dict]:
        ...
        while True:
            if last_evaluated_key:
                ...
                response = self.table.query(
                    KeyConditionExpression="PK = :pk", ExpressionAttributeValues={":pk": "WORD"}
                )
    ...
    async def get_progress(self, current_user_id: str, group: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            user_response = self.table.query(
                ...
                ExpressionAttributeValues={":pk": f"USER#{current_user_id}", ":sk_prefix": "WORD#"},
            )
            ...
            for level in target_levels:
                ...
                reviewable = sum(1 for item in level_user_items if self.datetime_utils.is_reviewable(item))
                if level_user_items:
                    # 学習済み単語の習熟度の平均を計算
                    gross_proficiency = sum(
                        float(item.get("proficiency_MJ", 0)) + float(item.get("proficiency_JM", 0))
                        for item in level_user_items
                    )
    ...
    async def get_progress_by_level(self, current_user_id: str, level: int) -> Optional[Dict]:
        ...
        try:
            ...
            if not level_words:
                # インデックス取得が失敗した場合のフォールバック
                all_words = self._get_all_words_with_pagination()
                level_words = [word for word in all_words if word.get("level") == level]
