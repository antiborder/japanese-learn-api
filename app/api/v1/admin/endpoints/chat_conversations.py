...
async def list_conversations(
    ...
    try:
        ...
        if category:
            items = [item for item in items if item.get("summary", {}).get("category") == category]
...
async def get_conversation(
    ...
    try:
        ...
        response = conversations_table.query(
            KeyConditionExpression="sessionId = :sessionId", ExpressionAttributeValues={":sessionId": session_id}
        )
...
async def get_conversation_stats(
    ...
    try:
        ...
        top_topics = (
            sorted([{"topic": k, "count": v} for k, v in topics_count.items()], key=lambda x: x["count"], reverse=True)[
                :10
            ]
            if topics_count
            else []
